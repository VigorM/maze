...
Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.float32, (21, 21)), ('action', np.int32), ('reward', np.float32), ('nonterminal', np.bool_)])
blank_trans = (0, np.zeros((21, 21), dtype=np.float32), 0, 0.0, False)
...
class ReplayMemory():
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    # print(states.shape)
    next_states = torch.tensor(all_states[:, self.n: self.n + self.history - 1], device=self.device, dtype=torch.float32).div_(255)
