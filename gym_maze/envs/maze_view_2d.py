...
class MazeView2D:
    ...
    def __view_update(self, mode="human"):
        if not self.__game_over:
            ...
            return
            # 색상정보를 numpy로 출력(생략)
            # np.flipud(np.rot90(pygame.surfarray.array3d(pygame.display.get_surface())))
    ...
    def __draw_maze(self):
        ...
        # horizontal wall  
        for i in range(1, self.maze.MAZE_H):
            for j in range(1, 2*self.maze.MAZE_H):
                map[2*i][j] = 1
        ...
        # vertical wall(1)
        for i in range(1, self.maze.MAZE_W):
            for j in range(1, 2*self.maze.MAZE_W):
                map[j][2*i] = 1
        ...
        for x in range(len(self.maze.maze_cells)):
            for y in range(len(self.maze.maze_cells[x])):
                ...
                for dir, open in walls_status.items():
                    if open:
                        dirs += dir
                    # self.__view_update('human')
                self.__cover_walls(x, y, dirs)
                self.__map_break_walls(x, y, dirs)
                # self.__view_update('human')
         
        # # breaking the walls(map)
        # for x in range(len(self.maze.maze_cells)):
        #     for y in range(len(self.maze.maze_cells[x])):
        #         my_walls_status = self.maze.get_walls_status(self.maze.maze_cells[x, y])
        #         dirs = ""
        #         for dir, open in my_walls_status.items():
        #             if open:
        #                 dirs += dir
        #         self.__map_break_walls(x, y, dirs)
    ...
    def __map_break_walls(self, x, y, dirs):
        ...
        for i in range(2*self.maze.MAZE_H + 1):
            map[0][i] = 1
            map[i][0] = 1
            map[2*self.maze.MAZE_H][i] = 1
            map[i][2*self.maze.MAZE_H] = 1
    ...
    def __draw_robot(self, colour=(0, 150, 0), transparency=255):
        map = self.__map
        if self.__enable_render is False:
            return
    ...
    def __draw_goal(self, colour=(150, 0, 0), transparency=235):
        ...
        self.__map_colour_cell(self.goal)
    ...
    def __map_colour_cell(self, cell):
